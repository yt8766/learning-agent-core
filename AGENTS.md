...
- 涉及 LangGraph `interrupt(payload)` 的节点、工具节点或 agent graph 主链，必须先阅读并遵守 [Runtime Interrupts](/docs/packages/runtime/runtime-interrupts.md)。
- 不要用会吞掉控制流的 `try/catch` 包裹 `interrupt()`；如必须捕获外部错误，只处理明确错误并 `throw` 回其余错误。
- 同一个节点内的 `interrupt()` 调用数量和顺序必须稳定；不要用条件分支、非确定性逻辑或动态列表循环改变中断顺序。
- `interrupt` payload 与 resume 值必须保持 JSON 可序列化，不要传函数、类实例、连接、流、`Request`、`Response` 或带运行时状态的复杂对象。
- `interrupt()` 前的外部副作用必须幂等；非幂等写入、通知、扣费、发布、append 历史等默认放在中断之后或拆进后续节点。
...
- 只要本轮改动涉及前后端联调、跨包 API、SSE payload、DTO、事件、tool result、graph state 切片或其他会被多个模块消费的接口，必须先定义并确认接口文档，再开始前后端实现；这是硬门禁，禁止以“局部小改”“先试实现”“后面补文档”为理由跳过，也禁止前端和后端各自先写代码、再事后倒推协议。
...
- 前端开发、后端开发、联调、测试与文档更新都必须以同一份接口文档为准；接口发生变更时，先更新接口文档与 schema/contract，再同步修改后端实现、前端消费和验证，禁止让文档长期落后于真实协议。
- `agent-chat` 下一代会话化 Agent 执行协议以 [Agent Chat Runtime V2 API](/docs/contracts/api/agent-chat-runtime-v2.md) 为先行契约；涉及 ChatRun、view-stream、自动审查、自然语言确认、checkpoint 或事件投影的开发，必须先按该文档确认字段，再开始后端和前端实现。
...
- 以后每次完成功能、修复缺陷、调整链路、补齐约束或确认重要联调结论后，代码代理必须在同一轮自动同步更新对应文档，不能只改代码不沉淀，也不能把“稍后补文档”留给用户或后续任务。
- “自动更新 docs” 是交付闭环的一部分：只要本轮改动影响真实行为、接口契约、事件 payload、目录边界、运行链路、验证要求、配置方式、用户可见流程或后续 AI 的执行规范，就必须主动定位并更新相关 `docs/**`、模块 README、`AGENTS.md` 或专题规范；不能等待用户再次提醒。
- “自动删除旧内容” 是默认 cleanup 动作：发现与当前实现明显冲突、已经失去入口价值、会误导后续开发或与新文档重复分叉的旧说明、旧待办、旧入口、旧流程图、旧兼容注释时，必须在本轮删除；如果仍有历史参考价值，必须改写为最新实现或显式标注“过时 / 仅历史参考 / 正确入口是 ...”。
...
- 文档 cleanup 不限于新增文档所在目录：如果本轮实现让旧 API 文档、README、计划文档、集成说明、规范文档或测试指南失效，必须直接更新或删除原位置，禁止额外新增一份“补充说明”后留下旧入口继续误导。
...
- 交付前必须至少执行一次文档过时扫描：围绕本轮关键词、模块名、接口路径、事件名、旧目录名或旧能力名使用 `rg` 检索 `docs/**`、模块 README 与 `AGENTS.md`，清理命中的过时描述；纯文档改动也必须执行 `pnpm check:docs`。
- 交付时应在结果中明确说明：
  - 新增或更新了哪些文档
  - 是否清理了过时文档
  - 后续 AI 应优先阅读哪些文档
...
- [验证体系规范](/docs/packages/evals/verification-system-guidelines.md) 是当前仓库所有非纯文档改动的验证分流入口；只要本轮改动触达代码、配置、模板、脚手架、构建脚本或测试文件，就必须按该文档判断受影响范围、验证层级与命令组合，不允许跳过为“局部小改”“只改一个文件”或“只是重构”。
- 日常改动不要求每次修改代码后手动执行全量 `pnpm verify`；本地开发优先运行新增测试、受影响测试、对应类型检查与必要治理检查，本地提交链路与 main CI 负责更重的聚合验证收口。
- 只要本轮触达代码、配置、模板、脚手架、构建脚本或测试文件，交付前就必须对受影响范围补齐必要验证，不允许只改代码不校验。
- 五层验证固定为：
...
- 根级 `pnpm verify` 是重型全量收口入口，主要用于本地提交链路触发的聚合验证、main CI 全量验证，以及开发者明确需要全仓证明的场景；不要把它当成每次文件修改后的默认命令。
...
- 每次交付前，都必须按 [验证体系规范](/docs/packages/evals/verification-system-guidelines.md) 判断本轮需要补齐的受影响范围、层级、门槛和命令；禁止凭经验只跑 `test`、只跑 `build`、只跑单个 `tsc` 或只做手工验证后直接交付。
- 如果受影响范围验证或 hook / CI 中的 `pnpm verify` 被与本轮无关的既有红灯、外部服务、凭据、网络或环境问题阻断，仍必须对受影响范围逐层补齐必要验证，并在交付中明确说明：
  - 实际执行了哪些命令
  - 哪一层因什么 blocker 未能完成
  - blocker 是否属于本轮改动
- 纯文档改动至少执行 `pnpm check:docs`；如果文档改动同时伴随代码或配置改动，仍按受影响范围补齐对应验证层级。
...
- 每次交付前都必须补齐受影响范围的必要验证，或明确记录阻断原因；不要只改代码不校验，也不要把全量 `pnpm verify` 作为每次小改的固定动作。
