...
type TranslatedBriefing = z.infer<typeof translatedBriefingSchema>;
...
function createTranslator(context: RuntimeTechBriefingTranslateContext) {
  ...
  return async (input: { category: TechBriefingCategory; title: string; summary: string; sourceName: string }) => {
    return generateTranslatedObjectWithRetry(provider, {
      messages: [
        {
          role: 'system',
          content: [
...
async function generateTranslatedObjectWithRetry(
  provider: ILLMProvider,
  input: {
    messages: Parameters<ILLMProvider['generateObject']>[0];
    options: Parameters<ILLMProvider['generateObject']>[2];
  }
): Promise<TranslatedBriefing> {
  let lastError: unknown;
  for (let attempt = 1; attempt <= TRANSLATION_RETRY_ATTEMPTS; attempt += 1) {
    try {
      return await provider.generateObject(input.messages, translatedBriefingSchema, {
        ...input.options,
        disableRetry: attempt > 1 ? true : input.options.disableRetry
      });
    } catch (error) {
      lastError = error;
    }
  }
  throw lastError instanceof Error ? lastError : new Error('briefing_translation_failed');
}
