...
describe('executePatrolIntelRun', () => {
  ...
  it('runs the patrol pipeline from search tasks through queued deliveries', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'intel-patrol-service-'));
  ...
  it('reuses the existing signal and source when a later patrol run produces the same dedupe key and source', async () => {
    const dataDir = await mkdtemp(join(tmpdir(), 'intel-patrol-service-'));
    tempDirs.push(dataDir);
