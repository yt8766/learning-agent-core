...
export class ReviewerAgent extends BaseAgent {
  constructor(context: AgentRuntimeContext) {
    super(AgentRole.REVIEWER, context);
  }
  ...
  async review(
    ...
    const llmReview = executionResult
      ? await this.generateObject(
          [
            {
              role: 'system',
              content:
                '你是评审 Agent。请始终使用中文，判断执行结果应通过、重试还是阻断，并决定是否写入记忆、规则或技能候选。若本轮体现出新的聊天能力，也应倾向生成技能候选。'
            },
            {
              role: 'user',
    ...
    this.setStatus(decision === 'approved' ? 'completed' : 'failed');
