...
import {
  ...
  inferDispatchKind,
  toManagerPlan
} from '../supervisor/contracts/supervisor-plan-contract';
import {
...
import { generateObjectWithRetry, generateTextWithRetry, streamTextWithRetry } from '../../utils/llm-retry';
...
export class LibuRouterMinistry {
  ...
  async plan(): Promise<ManagerPlan> {
    ...
    if (this.context.llm.isConfigured()) {
      try {
        const output = await withReactiveContextRetry<SupervisorPlanOutput | null>({
  ...
  async replyDirectly(): Promise<string> {
    ...
    if (this.context.llm.isConfigured()) {
      try {
        streamed = await streamTextWithRetry({
          llm: this.context.llm,
    ...
    if (!streamed && this.context.llm.isConfigured()) {
      try {
        fallback = await generateTextWithRetry({
          llm: this.context.llm,
          messages: promptMessages,
          options: {
            role: 'manager',
  ...
  async finalize(
    ...
    if (this.context.llm.isConfigured()) {
      try {
        llmSummary = await streamTextWithRetry({
          llm: this.context.llm,
          messages: promptMessages,
          options: {
            role: 'manager',
    ...
    if (!llmSummary && this.context.llm.isConfigured()) {
      try {
        fallbackSummary = await generateTextWithRetry({
          llm: this.context.llm,
          messages: promptMessages,
          options: {
            role: 'manager',
