...
function resolveScenario(task: PlanningPolicyTaskLike, dto: CreateTaskDto): PlanningScenario {
  ...
  if (workflowId === 'data-report') return 'data-report';
...
function buildBaseAssumptions(scenario: PlanningScenario) {
  ...
  } else if (scenario === 'data-report') {
    assumptions.push('默认优先抽取共享搜索/配置骨架，再拆分单报表或多报表模块。');
  } else if (scenario === 'qa') {
...
function buildScenarioQuestions(scenario: PlanningScenario): PlanQuestionRecord[] {
  switch (scenario) {
    case 'ceo-review':
      return [
        {
    ...
    case 'data-report':
      return [
        {
          id: 'report_scope',
          question: '这一轮报表生成希望我先覆盖哪种范围？',
          questionType: 'direction',
          options: [
            { id: 'single_report', label: '单个报表', description: '先落一个完整报表模块，方便快速验证结构。' },
            { id: 'multi_report', label: '多个报表', description: '按统一模板同时生成多个报表模块。' },
            { id: 'shell_first', label: '先搭骨架', description: '先只生成共享容器、搜索区和切换结构。' }
          ],
          recommendedOptionId: 'single_report',
          allowFreeform: true,
          defaultAssumption: '默认先落一个完整报表模块，并保持可扩展到多个模块。',
          whyAsked: '这会直接影响这轮生成范围与目录拆分粒度。',
          impactOnPlan: '影响组件数量、数据结构与交付边界。'
        },
        {
          id: 'template_fidelity',
          question: '对参考模板的复用方式，你更倾向哪种？',
          questionType: 'tradeoff',
          options: [
            {
              id: 'strict_template',
              label: '严格贴模板',
              description: '优先保持 bonusCenterData 一类页面的组织方式。'
            },
            { id: 'adapted_template', label: '适配后复用', description: '保留骨架，但按本项目约定做适度收敛。' },
            { id: 'design_system_first', label: '优先本项目', description: '只借鉴结构，不强依赖外部模板细节。' }
          ],
          recommendedOptionId: 'adapted_template',
          allowFreeform: true,
          defaultAssumption: '默认保留模板骨架，同时按当前仓库规范做适配。',
          whyAsked: '模板贴合度会影响目录结构、命名和组件抽象层次。',
          impactOnPlan: '影响最终代码的相似度与长期可维护性。'
        }
      ];
    case 'qa':
      return [
...
export function buildPlanningPolicy(task: PlanningPolicyTaskLike, dto: CreateTaskDto): PlanningPolicy {
  ...
  return {
    ...
    questions: buildScenarioQuestions(scenario),
    autoResolved: buildBaseAutoResolved(task, workflow, dto),
    assumptions: buildBaseAssumptions(scenario),
    microBudget: {
      readOnlyToolLimit: scenario === 'qa' || scenario === 'browse' ? 2 : scenario === 'data-report' ? 4 : 3,
      readOnlyToolsUsed: 0,
