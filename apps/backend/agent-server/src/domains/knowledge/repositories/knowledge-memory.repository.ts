...
import type { KnowledgeBaseCreateInput, KnowledgePrincipalSelector, KnowledgeRepository } from './knowledge.repository';
...
export class KnowledgeMemoryRepository implements KnowledgeRepository {
  ...
  async createBase(input: KnowledgeBaseCreateInput): Promise<KnowledgeBase> {
    const createdByUserId = input.createdByUserId ?? input.ownerId;
    if (!createdByUserId) {
    ...
    await this.addMember({ knowledgeBaseId: base.id, userId: createdByUserId, role: 'owner' });
    return base;
  }

  async listBases(input: KnowledgePrincipalSelector): Promise<KnowledgeBase[]> {
    return this.listBasesForUser(input.userId);
  }
