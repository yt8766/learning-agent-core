...
import type { KnowledgeBase, KnowledgeBaseMember, KnowledgeBaseMemberCreateRequest } from '@agent/core';
...
import {
  ensureChatConversationForUser,
  requiredRow,
  stringifyJsonParam
} from './knowledge-postgres.repository.helpers';
import type { KnowledgeBaseCreateInput, KnowledgeRepository } from './knowledge.repository';
...
export class PostgresKnowledgeRepository implements KnowledgeRepository {
  ...
  async createBase(input: KnowledgeBaseCreateInput): Promise<KnowledgeBase> {
    const id = input.id ?? `kb_${randomUUID()}`;
    const ownerId = input.createdByUserId ?? input.ownerId;
    if (!ownerId) {
      throw new Error('knowledge_base_owner_required');
    }
    const result = await this.client.query(
      ...
      [id, input.name, input.description ?? '', ownerId]
    );
    const base = mapBase(requiredRow(result.rows[0], 'knowledge base'));
    await this.addMember({ knowledgeBaseId: base.id, userId: ownerId, role: 'owner' });
    return base;
  }

  async listBases(selector: { userId: string }): Promise<KnowledgeBase[]> {
    return this.listBasesForUser(selector.userId);
  }
  ...
  async appendChatMessage(input: CreateKnowledgeChatMessageRecordInput): Promise<KnowledgeChatMessageRecord> {
    ...
    await ensureChatConversationForUser(this.client, messageInput.conversationId, messageInput.userId);
    const result = await this.client.query(
  ...
  async updateMessageFeedback(messageId: string, feedback: unknown): Promise<KnowledgeChatMessageRecord | undefined> {
    ...
    return result.rows[0] ? mapChatMessage(requiredRow(result.rows[0], 'knowledge chat message')) : undefined;
  }
}
