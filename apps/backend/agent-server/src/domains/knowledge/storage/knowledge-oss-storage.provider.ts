...
import { KNOWLEDGE_OSS_STORAGE } from '../knowledge-domain.tokens';
import { KnowledgeServiceError } from '../services/knowledge-service.error';
import { InMemoryOssStorageProvider } from './in-memory-oss-storage.provider';
import type {
  OssListObjectsInput,
...
type KnowledgeOssStorageEnv = Partial<
  Record<
    | 'KNOWLEDGE_OSS_PROVIDER'
    | 'ALIYUN_OSS_BUCKET'
...
export type KnowledgeOssStorageProvider = FactoryProvider<OssStorageProvider> & {
  useFactory: () => OssStorageProvider;
};
...
export function createKnowledgeOssStorageProvider(
  options: { env?: KnowledgeOssStorageEnv } = {}
): KnowledgeOssStorageProvider {
  return {
    provide: KNOWLEDGE_OSS_STORAGE,
    useFactory: (): OssStorageProvider => {
      const env = options.env ?? process.env;
      const aliyun = createAliyunOssStorageProviderFromEnv(env);
      return aliyun ?? new InMemoryOssStorageProvider();
    }
  };
}

export function createAliyunOssStorageProviderFromEnv(
  env: KnowledgeOssStorageEnv
): AliyunOssStorageProvider | undefined {
...
function loadAliOssClient(): AliOssConstructor {
  return createRequire(__filename)('ali-oss') as AliOssConstructor;
}
