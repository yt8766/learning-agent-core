...
import { buildInteractiveCard } from './runtime-tech-briefing-localize-card';
import {
...
function appendSecuritySummaryItem(lines: string[], category: TechBriefingCategory, item: TechBriefingItem) {
  ...
  lines.push(
    `### ${priorityEmoji(item.priorityCode)}【${item.priorityCode ?? 'P2'} · ${formatActionDeadline(item).split('（')[0].replace(/^.\s*/, '')}】${trimText(item.cleanTitle ?? item.title, 96)}`
  );
