...
} from '../../../../src/runtime/domain/connectors/runtime-connector-governance-state';

describe('runtime connector governance state', () => {
  ...
  it('maps template ids to configured connector ids and upserts configured connector records', () => {
    ...
    expect(
      setConfiguredConnectorRecord(snapshot, {
        ...
        actor: 'tester',
        enabled: false
      } as any).governance.configuredConnectors
    ).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
