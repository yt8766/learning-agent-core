...
import { KnowledgeServerSearchServiceAdapter } from '../../src/knowledge/rag/knowledge-server-search-service.adapter';
...
describe('KnowledgeServerSearchServiceAdapter', () => {
  it('uses SDK embeddings and vector search before keyword fallback', async () => {
    ...
    const runtime = enabledRuntime({
      embedText: vi.fn(async () => ({ embedding: [0.1, 0.2], model: 'fake-embedding' })),
  ...
  it('falls back to keyword and substring search when vector search fails', async () => {
    const repository = new InMemoryKnowledgeRepository();
    const runtime = enabledRuntime({
      search: vi.fn(async () => {
    ...
    await seedDocument(repository, {
      ...
      content: '检索前规划会生成 query rewrite 和 variants。'
    });

    const result = await new KnowledgeServerSearchServiceAdapter(repository, runtime).search({
      query: '检索前规划',
      filters: { knowledgeBaseIds: ['kb_fallback'] },
      limit: 5
    });
  ...
  it('does not return weak Chinese substring matches as grounded hits', async () => {
    const repository = new InMemoryKnowledgeRepository();
    const runtime = enabledRuntime({
      search: vi.fn(async () => ({ hits: [] }))
    });
    await seedDocument(repository, {
      baseId: 'kb_weak_cn',
      documentId: 'doc_weak_cn',
      chunkId: 'chunk_weak_cn',
      content: '风险矩阵用于评估配置变更影响。'
    });

    const result = await new KnowledgeServerSearchServiceAdapter(repository, runtime).search({
      query: '风险动作审批',
      filters: { knowledgeBaseIds: ['kb_weak_cn'] },
      limit: 5
    });
  ...
  it('ignores vector hits that cannot be mapped to repository chunks', async () => {
    const repository = new InMemoryKnowledgeRepository();
    const runtime = enabledRuntime({
      search: vi.fn(async () => ({ hits: [{ id: 'missing_chunk', score: 0.99 }] }))
    });
    await seedDocument(repository, {
      baseId: 'kb_missing_vector_hit',
      documentId: 'doc_missing_vector_hit',
      chunkId: 'chunk_missing_vector_hit',
      content: 'known repository content'
    });

    const result = await new KnowledgeServerSearchServiceAdapter(repository, runtime).search({
      query: 'unknown semantic query',
      filters: { knowledgeBaseIds: ['kb_missing_vector_hit'] },
      limit: 5
    });
