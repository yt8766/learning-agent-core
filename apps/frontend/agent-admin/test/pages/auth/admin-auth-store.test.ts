...
describe('admin auth store', () => {
  ...
  it('exposes the auth snapshot through a Zustand store selector API', async () => {
    const { adminAuthStore, useAdminAuthStore } = await import('@/pages/auth/store/admin-auth-store');

    adminAuthStore.setAuthenticated(account(), tokenPair('access-zustand', 'refresh-zustand'), { persist: false });

    expect(useAdminAuthStore.getState().snapshot).toEqual(
      expect.objectContaining({
        state: 'authenticated',
        accessToken: 'access-zustand',
        refreshToken: 'refresh-zustand'
      })
    );
  });
