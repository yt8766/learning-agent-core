...
export function AuthFilesPage() {
  ...
  const setCurrentModePageSize = useCallback(
    (next: number) => {
      setPageSizeByMode(current => (compactMode ? { ...current, compact: next } : { ...current, regular: next }));
    },
    [compactMode]
  );
    ...
    return filesMatchingStatusFilters.filter(item => {
      ...
      const matchSearch =
        ...
        [item.name, item.type, item.provider].some(value => {
          ...
          return wildcardSearch ? wildcardSearch.test(content) : content.toLowerCase().includes(normalizedTerm);
        });
  ...
  const selectablePageItems = useMemo(() => pageItems.filter(file => !isRuntimeOnlyAuthFile(file)), [pageItems]);
  const selectableFilteredItems = useMemo(() => sorted.filter(file => !isRuntimeOnlyAuthFile(file)), [sorted]);
  ...
  const batchStatusButtonsDisabled =
    disableControls || selectedNames.length === 0 || batchStatusUpdating || selectedHasStatusUpdating;
  ...
  const deleteAllButtonLabel = (() => {
    ...
    return filter === 'all' ? t('auth_files.delete_all_button') : `${t('common.delete')} ${getTypeLabel(t, filter)}`;
  })();
                  ...
                  <div className={styles.filterToggleGroup}>
                    <div className={styles.filterToggleCard}>
                      <ToggleSwitch
                        ...
                        label={<span className={styles.filterToggleLabel}>{t('auth_files.problem_filter_only')}</span>}
                      />
                    </div>
                    <div className={styles.filterToggleCard}>
                      <ToggleSwitch
                        ...
                        label={<span className={styles.filterToggleLabel}>{t('auth_files.disabled_filter_only')}</span>}
                      />
                    </div>
                    <div className={styles.filterToggleCard}>
                      <ToggleSwitch
                        ...
                        label={<span className={styles.filterToggleLabel}>{t('auth_files.compact_mode_label')}</span>}
                      />
                    </div>
                  </div>
                </div>
