...
const normalizeApiKeyEntry = (entry: unknown): ApiKeyEntry | null => {
  ...
  const authIndex = normalizeAuthIndex(record?.['auth-index'] ?? record?.authIndex ?? record?.['auth_index']);
...
const normalizeProviderKeyConfig = (item: unknown): ProviderKeyConfig | null => {
  ...
  if (isRecord(cloakRaw)) {
    ...
    const strictMode = normalizeBoolean(cloakRaw['strict-mode'] ?? cloakRaw.strictMode ?? cloakRaw.strict_mode);
...
const normalizeOpenAIProvider = (provider: unknown): OpenAIProviderConfig | null => {
  ...
  const authIndex = normalizeAuthIndex(provider['auth-index'] ?? provider.authIndex ?? provider['auth_index']);
  if (authIndex) result.authIndex = authIndex;
  return result;
};
...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  if (isRecord(quota)) {
    config.quotaExceeded = {
      ...
      antigravityCredits: normalizeBoolean(quota['antigravity-credits'] ?? quota.antigravityCredits)
    };
  }
  ...
  if (Array.isArray(geminiList)) {
    config.geminiApiKeys = geminiList.map(item => normalizeGeminiKeyConfig(item)).filter(Boolean) as GeminiKeyConfig[];
  }
