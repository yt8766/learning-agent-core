...
vi.mock('@ant-design/x/es/bubble', () => ({
  default: {
    List({ items }: { items: Array<{ content?: React.ReactNode; footer?: React.ReactNode; key: string }> }) {
      return (
        <div>
          {items.map(item => (
            <article key={item.key}>
              <div>{item.content}</div>
              {item.footer ? <footer>{item.footer}</footer> : null}
            </article>
          ))}
        </div>
      );
...
vi.mock('@ant-design/x/es/sender', () => ({
  default({ onSubmit, value }: { onSubmit: (message: string) => void; value?: string }) {
...
describe('knowledge production workflow pages', () => {
  ...
  it('confirms before deleting a document from DocumentsPage', async () => {
    ...
    expect(testState.renderedPopconfirms).toHaveLength(1);
    expect(testState.renderedPopconfirms[0]).toMatchObject({
      title: '删除文档？',
      okText: '确认删除',
      cancelText: '取消',
      okButtonProps: { danger: true }
    });
    expect(String(testState.renderedPopconfirms[0]?.description)).toContain(knowledgeDocument.title);
    expect(client.deleteDocument).not.toHaveBeenCalled();

    await act(async () => {
      testState.renderedPopconfirms[0]?.onConfirm?.();
    });
    await flushEffects();
  ...
  it('uses the first accessible knowledge base as the DocumentsPage upload target', () => {
