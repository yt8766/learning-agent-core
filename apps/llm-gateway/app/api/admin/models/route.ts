...
export async function GET(request: Request) {
  try {
    ...
    return Response.json(await getAdminProviderModelRouteServiceForRoutes().listModels());
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    await requireAdminAccess(request.headers.get('authorization'));
    return Response.json(await getAdminProviderModelRouteServiceForRoutes().createModel(await request.json()));
  } catch (error) {
    return adminProviderModelErrorResponse(error);
  }
}
