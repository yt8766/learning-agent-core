...
`KnowledgeModule` 注册 `KNOWLEDGE_SDK_RUNTIME` token，默认由 `src/knowledge/runtime/knowledge-sdk-runtime.provider.ts` 创建。该 provider 只负责建立可注入 runtime 边界，当前 ingestion/chat 仍未接入完整 SDK 主链。

本地开发和测试环境可以完全不配置 SDK env。`DATABASE_URL`、`KNOWLEDGE_CHAT_MODEL`、`KNOWLEDGE_EMBEDDING_MODEL`、`KNOWLEDGE_LLM_API_KEY` 全部缺失时，provider 返回：

```ts
{ enabled: false, reason: 'missing_env', missingEnv: [...], runtime: null }
```

只要出现任一 SDK 相关配置但缺少关键项，启动期会抛出 `KnowledgeSdkRuntimeProviderConfigError`，避免半配置状态静默降级。配置齐全时，provider 会先通过 `createKnowledgeDatabaseClient({ databaseUrl })` 创建项目自有 `PostgresKnowledgeClient`，执行 `KNOWLEDGE_SCHEMA_SQL`，然后调用 `@agent/knowledge/node` 的 `createDefaultKnowledgeSdkRuntime()`。
...
- `upsert_knowledge_chunks` -> `select * from upsert_knowledge_chunks($1, $2, $3, $4::jsonb)`
- `match_knowledge_chunks` -> `select * from match_knowledge_chunks($1, $2, $3, $4::vector, $5, $6::jsonb)`
- `delete_knowledge_document_chunks` -> `select * from delete_knowledge_document_chunks($1, $2, $3)`
...
`KnowledgeIngestionWorker` 目前是最小同步闭环：读取 upload 内容，按段落切 Markdown/TXT，保存 chunks，并把 job 标记为 `succeeded`。真实异步队列、embedding provider 和向量/关键词索引 provider 后续应继续接在 worker/provider 边界，不要回到 controller 内联处理。
