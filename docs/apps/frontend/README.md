...
本目录镜像 `apps/frontend/*`，向下区分 `agent-chat`、`agent-admin`、独立 `knowledge`、`agent-gateway` 与私用 `llm-gateway` 应用文档。
...
4. [Agent Gateway 前端文档](/docs/apps/frontend/agent-gateway/README.md)
5. [llm-gateway 文档目录](/docs/apps/frontend/llm-gateway/README.md)
6. [前端规范](/docs/conventions/frontend-conventions.md)
7. [API 文档目录](/docs/contracts/api/README.md)
8. [前后端集成链路](/docs/integration/frontend-backend-integration.md)

改前端前先确认：

- 当前改动是在前线执行面 `agent-chat`，还是后台治理面 `agent-admin`
- 当前改动是否属于独立知识库前端 `knowledge`，它只面向知识库运营、RAG 验证、观测和评测闭环
- 当前改动是否属于独立中转控制台 `agent-gateway`，它只负责上游方、认证文件、配额、日志、探测与 token 处理链路，不接入开发自治 runtime / agent 主链
...
- Agent Gateway 前端入口：[agent-gateway/README.md](/docs/apps/frontend/agent-gateway/README.md)
...
- [Agent Gateway 前端文档](/docs/apps/frontend/agent-gateway/README.md)
