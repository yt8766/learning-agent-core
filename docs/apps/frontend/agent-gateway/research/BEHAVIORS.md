...
> 2026-05-13 起，`apps/frontend/agent-gateway` 已直接采用 `/Users/dev/Desktop/Cli-Proxy-API-Management-Center/src`。本文记录的是旧 `src/app/GatewayWorkspace` 混合架构的行为沉淀，仅保留历史参考；当前入口以 [Agent Gateway Frontend](/docs/apps/frontend/agent-gateway/README.md) 为准。
...
- AI 提供商：`/ai-providers` 使用纵向 provider 大卡；每张卡顶部为参考项目 provider SVG + 标题 + 灰色添加按钮，内容区为虚线空状态；已有配置的 provider 卡片展示编辑、启停、保存、测试模型、刷新模型和删除入口，所有远端动作必须在卡片内显示 loading / success / error；底部固定 `provider-floating-nav` 图标浮条，hover 时图标按钮上移 2px。
