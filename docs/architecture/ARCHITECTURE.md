...
- [system flow current state](/docs/maps/system-flow-current-state.md)
- [Runtime 分层 ADR](/docs/packages/runtime/runtime-layering-adr.md)
...
packages/runtime           = Runtime Kernel 与 Agent SDK 基础宿主
...
- [Agent Memory Architecture](/docs/packages/memory/agent-memory-architecture.md)
...
仓库级代理 skills 当前默认放在 `.agents/skills/*`，按 `SKILL.md + frontmatter` 规范组织，并由本地 loader 直接转成运行时 manifest。
...
`packages/knowledge/src/contracts/indexing/` 是所有 indexing pipeline 参与方的共享契约层，调用方通过 `@agent/knowledge` 消费，不再从 `packages/core/src/knowledge/*` 取 indexing contract。
...
详见：[Knowledge Indexing 契约规范](/docs/packages/knowledge/indexing-contract-guidelines.md)
...
- 目录：`.agents/skills/*`
- 用途：给代码代理看的工作流规范、脚本和参考资料；本仓库 Codex 从 `.agents/skills` 发现技能
...
- `scripts/` 放可执行脚本
- `assets/` 放模板或静态资源
- 仓库级代理 skill 不应与运行时 `packages/skill` / `@agent/skill` 混合
- 新增代理工作流时，优先考虑放进 `.agents/skills/`，而不是塞进随机文档目录
...
3. [API 文档目录](/docs/contracts/api/README.md)
4. [前后端集成链路](/docs/integration/frontend-backend-integration.md)
5. [后端规范](/docs/conventions/backend-conventions.md)
6. [前端规范](/docs/conventions/frontend-conventions.md)
