...
| `GET`  | `/api/platform/intelligence/overview`           | 无                                                                    | `IntelligenceOverviewProjection`                          | Intelligence 最近信号与待审候选投影。                              |
| `POST` | `/api/platform/intelligence/:channel/force-run` | path: `channel: IntelligenceChannel`                                  | `IntelligenceForceRunResult`                              | 立即触发指定 Intelligence 频道运行；`channel` 由后端 schema 校验。 |

`/api/platform/intelligence/*` 是 Tech & AI Intelligence 的 Postgres-first 治理入口，使用 `@agent/core`
里的 `IntelligenceChannel` 与 `IntelligenceOverviewProjection`。旧 `/api/platform/briefings/*` 已下线，新增抓取、
知识候选入库和 Admin 强制运行都必须走 `/api/platform/intelligence/*`。

`IntelligenceForceRunResult`：

```ts
type IntelligenceForceRunResult = {
  ok: true;
