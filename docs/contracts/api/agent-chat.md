...
| `POST`   | `/api/chat/sessions`                     | body: `{ message?: string; title?: string; channelIdentity?: ChannelIdentity }`                                                                                                                        | `ChatSessionRecord`                            | 创建会话并写入 `agent_session_id` cookie。`message` 只作为创建上下文，不替代 `POST /api/chat/messages`。`title: "新会话"` / `"新对话"` 均视为占位标题，返回 `titleSource: "placeholder"`，后续首轮回复仍可自动命名。                                                                          |
...
- 触发条件：无 slash workflow 命令、无显式 skill / connector / capability attachment、无执行/检索/发布/审批/报表/代码修改等高意图信号。
...
Task Trajectory 不新增新的 SSE event type。后端通过现有稳定事件类型投影轨迹，避免破坏旧消费者：

- 单步轨迹使用 `execution_step_started`、`execution_step_completed`、`execution_step_blocked`、`execution_step_resumed`。
- 完整轨迹快照使用 `node_progress`，并带 `payload.projection = "task_trajectory"`。
- 单步投影 payload 必须包含 `projection = "trajectory_step"`、`sourceEventId`、`sourceEventType`、`taskId`、`trajectoryStep`。
- 完整快照 payload 必须包含 `projection = "task_trajectory"`、`taskId`、`taskTrajectory`。

`trajectoryStep` 必须能被 `TrajectoryStepRecordSchema` parse；`taskTrajectory` 必须能被 `TaskTrajectoryRecordSchema` parse。后端投影 helper 入口为 `apps/backend/agent-server/src/chat/chat-trajectory-events.adapter.ts`，前端消费 helper 入口为 `apps/frontend/agent-chat/src/lib/chat-trajectory-projections.ts`。
...
`chat_response_steps` snapshot may carry optional inline Agent OS presentation fields:
...
- `agentOsGroups[].steps`: user-readable steps after adapter conversion; internal `nodeId`, `fromNodeId`, and `toNodeId` are not main visual copy

Old clients can ignore these fields. New clients must tolerate their absence and may derive fallback display mode and groups from step `phase`, `target`, `agentScope`, and `sourceEventType`. Current `agent-chat` renders `answer_only` as ordinary `思考中 / 已思考`; `agent_execution` renders one inline `处理中 / 已处理` Agent OS entry inside the assistant message.

`ChatResponseStepTargetSchema` is a discriminated union. `kind = "file"` requires `path`; `kind = "url"` requires `href`; other target kinds only carry `label`. Snapshot payloads must contain steps whose `sessionId` and `messageId` match the snapshot wrapper, and `summary.*Count` must equal the counts derived from `steps`. Incremental payload actions must align with step status: `started -> running`, `updated -> queued | running`, `completed -> completed`, `blocked -> blocked`, `failed -> failed`, and `cancelled -> cancelled`.

`ChatResponseStepRecord` 还可携带 Agent / 节点展示元数据，字段均为可选以兼容旧事件：`agentScope = "main" | "sub" | "system"`、`agentId`、`agentLabel`、`ownerLabel`、`nodeId`、`nodeLabel`、`fromNodeId`、`toNodeId`、`durationMs`。前端用这些字段区分主 Agent、子 Agent、系统步骤，以及展示节点流转；字段缺失时由后端 projection 使用事件类型补中文 fallback。
