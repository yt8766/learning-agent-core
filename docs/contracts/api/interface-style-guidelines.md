...
状态：current
文档类型：guide
适用范围：`apps/backend/agent-server`、跨端 API 与 SSE 契约
最后核对：2026-04-25
...
| LLM 调用命令         | REST command                      | 由宿主 API 封装模型调用，不向前端透出 provider 原始协议       |
| LLM streaming        | SSE                               | 统一投影为项目事件，不直接透出 provider 原始 chunk            |
