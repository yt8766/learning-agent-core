...
> 统一后端迁移说明：`agent-server` 已新增 `src/domains/knowledge` route shell，目标是把 standalone `knowledge-server` 合并进单一后端宿主。统一后端已接入 base/member、document/upload、非流式 Chat Lab RAG、conversation 与 feedback endpoint；迁移完成前，standalone `knowledge-server` 和旧 `agent-server/src/knowledge` 仍保留兼容价值。新增后端能力默认向 `agent-server/src/domains/knowledge` 收敛。
...
`POST /api/knowledge/chat` returns `answer`, grounded `citations`, `route`, `diagnostics`, `traceId`, and optional `usage`.
