...
本检查基于 [当前 core 包规范](/docs/core/current-core-package-guidelines.md) 对 `packages/core` 的当前实现做现实核对，重点判断三件事：
...
- 最新全仓结果已经满足：
  ...
  - `packages/shared` 目录本体已删除，只保留 `docs/shared/*` 作为历史归档
...
- `docs/package-architecture-guidelines.md` 里还残留旧 `agent-core` 说法，需要同步更新成 `runtime / agents/*`
- `docs/core/current-core-package-audit.md` 旧版本里对 `packages/core/package.json` 依赖状态的描述已经过时，不能继续保留
...
4. 每次迁移后同步更新 `docs/core/*`、`docs/shared/*` 与包分层规范
