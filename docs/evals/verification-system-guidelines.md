...
- workspace smoke 当前不默认并入 `pnpm verify` 或 `pnpm verify:affected`，避免在 PR 阶段误阻塞；main CI 已将 `pnpm test:workspace:smoke` 作为阻塞项执行，需要本地验证时也应显式执行该命令
...
- PR 增量校验：`/.github/workflows/pr-check.yml`
  - 代码改动默认按 `pnpm verify:affected` 的层级执行：先跑 `verify:governance + test:spec:affected`，再并发跑 `lint:prettier:affected + lint:eslint:affected + typecheck:affected + test:unit:affected + test:demo:affected + test:integration:affected + test:workspace:integration:affected`
  - PR 阶段 workspace integration 是阻塞项；workspace smoke 暂不阻塞 PR，避免高层 smoke 在成熟前拉长增量验证关键路径
