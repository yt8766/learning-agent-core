...
- **workspace smoke 不阻塞 PR**：当前 workspace smoke 作为本地验证补充，不纳入 PR 阻塞条件
- **workspace smoke 不阻塞 main**：当前 main 分支的 CI 仍以现有 `pnpm verify` 为主，不强制执行 workspace smoke
- workspace integration 的执行入口与宿主内 integration 分离：
  - 宿主内：`pnpm test:integration`（现有入口）
  - workspace：`pnpm test:workspace:integration`（后续接入）
  - workspace smoke：`pnpm test:workspace:smoke`（后续接入）

### 12.2 后续阶段（Phase 3）

- workspace smoke 纳入 main 分支 CI，作为独立 job 执行全量验证
- workspace integration 纳入本地 `pnpm verify` 和 CI affected 计算策略
- 引入外部环境 guard：需要外部服务的 smoke/integration 通过环境变量显式 skip，防止在无外部服务的 CI 环境失败
- PR 是否引入 workspace smoke 阻塞，视补测成熟度在 Phase 3 评估决定
...
- ✅ 更新 `docs/test-conventions.md`，补充根级 `test/` 作为 workspace test host 的正式说明
- ✅ 更新 `docs/evals/verification-system-guidelines.md`，补充 workspace test host 引用
- ✅ 更新 `README.md`，补充根级 `test/` 目录职责说明

### Phase 2：第一批 integration 用例（已完成）
...
6. ⏳ **backend chat SSE smoke** → 待补充 `test/smoke/backend/`
