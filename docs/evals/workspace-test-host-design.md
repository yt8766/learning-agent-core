...
- 不把 workspace smoke 作为 PR 阶段阻塞项
...
- **workspace smoke 不阻塞 PR**：当前 workspace smoke 不纳入 PR 阻塞条件，避免高层 smoke 在成熟前拉长增量验证关键路径
- **workspace smoke 阻塞 main**：main 全量校验通过 `pnpm test:workspace:smoke` 执行 workspace smoke
...
- workspace smoke 已纳入 main 分支 CI，后续评估是否进入 PR 阻塞链
