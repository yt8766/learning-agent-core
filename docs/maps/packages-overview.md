...
| `packages/skill-runtime`    | 运行时 skill catalog、install、policy、registry                                   | `@agent/skill-runtime`      | 仓库代理技能 `.agents/skills/*`、graph 主流程、tool 主编排  | `catalog/`、`install/`、`policies/` 继续补 schema/contract                                                                      |
...
- `docs/packages/skill-runtime/*` 现在专门承载 `packages/skill-runtime` 文档；不要再与 `docs/skills/*` 混用
