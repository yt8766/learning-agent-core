...
- 分层与依赖约定：[package-architecture-guidelines.md](/docs/conventions/package-architecture-guidelines.md)
- 全局 runtime 分层 ADR：[runtime-layering-adr.md](/docs/packages/runtime/runtime-layering-adr.md)
- compat 收缩候选：[package-compat-sunset-candidates.md](/docs/packages/core/package-compat-sunset-candidates.md)
- contract 边界：[core/README.md](/docs/packages/core/README.md)
...
- `packages/runtime`
  - Runtime Kernel：graph 执行、session、checkpoint、approval/recovery、interrupt、observability、Agent SDK 基础能力与抽象 registry contract 的真实实现入口
...
| `packages/runtime`          | Runtime Kernel、Agent SDK、descriptor、capability 声明、基础执行 helper           | `@agent/runtime`            | 官方 Agent 默认组合、app view model、controller                        | 继续承接 agent foundation、capability dispatch contract 与 planner strategy selector    |
| `packages/platform-runtime` | 可注入 platform facade、agent registry、backend/worker 共享启动线                 | `@agent/platform-runtime`   | HTTP controller、worker loop、Agent graph 主实现、官方 agents 直接依赖 | 承接通用 registry、只读 metadata facade、default runtime facade 与 app 层注入边界       |
...
| `packages/skill`            | 运行时 skill catalog、install、policy、registry                                   | `@agent/skill`              | 仓库代理技能 `.agents/skills/*`、graph 主流程、tool 主编排             | `catalog/`、`install/`、`policies/` 继续补 schema/contract                              |
...
- backend agents 组合根的 `createOfficialAgentRegistry()` 现已提供默认官方 agent descriptor、capability 与 specialist domain lookup；`createOfficialRuntimeAgentDependencies()` 也会优先按 capability contract 解析官方模块，并把 specialist route enrich 为官方 agent 匹配线索，后续 supervisor capability dispatch 应优先挂到这层 contract
...
- `packages/runtime` 现已承载通用 `AgentDescriptor` / `AgentProvider` / `AgentRegistry` contract；`packages/platform-runtime` 只保留通用 registry 实现与 facade re-export
...
- `docs/archive/*` 仅保留已删除包或旧方案的迁移历史归档，不属于当前 `packages/*` 或 `agents/*` 阅读路径；新增当前实现文档必须落到真实宿主目录。
...
- 已进一步补出 facade contract 的包：
  ...
  - `packages/skill`：`contracts/skill-facade.ts`
...
- 继续删除前应先看
  - [Compat 入口收缩候选](/docs/packages/core/package-compat-sunset-candidates.md)

补充说明：

- `.agents/skills/` 是给代码代理读取的仓库级技能目录，不属于这里的运行时包体系
- `docs/packages/skill/*` 现在专门承载 `packages/skill` 文档；不要再与 `docs/skills/*` 混用
...
1. [Runtime 分层 ADR](/docs/packages/runtime/runtime-layering-adr.md)
2. [core 文档目录](/docs/packages/core/README.md)
3. [Packages 分层与依赖约定](/docs/conventions/package-architecture-guidelines.md)
4. [目录地图](/docs/maps/repo-directory-overview.md)
5. [runtime 文档目录](/docs/packages/runtime/README.md)
6. [report-kit 包结构规范](/docs/packages/report-kit/package-structure-guidelines.md)
7. [templates 包结构规范](/docs/packages/templates/package-structure-guidelines.md)
8. [tools 包结构规范](/docs/packages/tools/package-structure-guidelines.md)
