...
- `docs/`
  - 架构、规范、模块说明、联调结论。
- `.agents/skills/`
  - 给 Codex / Claude Code 读取的仓库级代理技能；本仓库 Codex 从这里发现技能。
- `scripts/`
  - 仓库级维护脚本与检查脚本。
- `mock/`
  - 本地联调或测试辅助资源。
...
- 根目录只保留 `README.md` 和一级分类目录；具体主题文档必须落到对应分类下。
- `docs/architecture/`
  - 长期架构方向与核心工作流。
- `docs/maps/`
  - 仓库地图、apps/packages/data 总览与系统流概览。
- `docs/architecture/ARCHITECTURE.md`
  - 长期架构方向与“皇帝-首辅-六部”主线。
- `docs/conventions/`
  - 工程规范、GitHub Flow、前后端规范、测试规范、LangGraph 结构规范。
- `docs/apps/backend/agent-server/`、`docs/apps/backend/worker/`
  - 后端服务与 worker 应用文档。
- `docs/apps/frontend/agent-chat/`、`docs/apps/frontend/agent-admin/`、`docs/apps/frontend/llm-gateway/`
  - 前端应用文档，镜像 `apps/frontend/*`。
- `docs/contracts/api/`
  - API 契约文档。
- `docs/integration/`
  - 前后端联调、跨模块链路和集成说明。
...
- `.agents/skills/code-review`
  - 面向代码审查与回归风险检查的代理技能。
- `.agents/skills/learning-flow-audit`
  - 面向学习链路审查的代理技能。
- `.agents/skills/openclaw-workspace-audit`
