...
- `createMiniMaxMcpSkillProvider()`
  - 默认 servers：`minimax-mcp`、`minimax-token-plan-mcp`
  - 默认 transport：`stdio`
  - 默认命令：`uvx minimax-mcp -y`、`uvx minimax-coding-plan-mcp -y`
  - secret：`MINIMAX_API_KEY`
  - 能力：音频、声音列表、声音克隆、视频、图片、视频任务查询、音乐、voice design、Token Plan `web_search`、Token Plan `understand_image`
...
- MiniMax Token Plan MCP 的项目侧稳定 capability id 是 `minimax:web_search` 与 `minimax:understand_image`；真实 MCP toolName 保持官方名称 `web_search` / `understand_image`。
