...
| platform-runtime public facade    | `test/integration/packages/platform-runtime-public-entrypoints.int-spec.ts` | 已补 baseline | 覆盖默认 facade、自定义 facade、官方 registry、runtime dependencies 与 core metadata schema 协同。                    |
...
| runtime graph contract projection | `test/integration/runtime/runtime-graph-contract-projection.int-spec.ts`    | 已补 baseline | 覆盖 runtime graph 完成路径和 pending approval 分支投影到 core task / checkpoint contract。                           |
...
| workspace smoke topology          | `test/smoke/workspace/workspace-smoke-entrypoints.smoke.ts`                 | 已补 baseline | 覆盖根级 smoke 脚本接线、verify 接线、lane 覆盖与 `*.smoke.ts(x)` 命名约束。                                          |
...
- runtime 主链当前有最小 graph invoke 和 core contract projection，尚未覆盖 evidence、learning suggestion、skill reuse badge 的完整真实状态生成。
