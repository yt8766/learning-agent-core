...
- [测试覆盖率基线](/docs/packages/evals/testing-coverage-baseline.md)
- [Prompt Regression And Thresholds](/docs/packages/evals/prompt-regression-and-thresholds.md)
...
仓库级 workspace test host 的正式设计见 [Workspace Test Host 设计](/docs/packages/evals/workspace-test-host-design.md)，其根级落点为 `test/`（单数），与各宿主的 `test/` 语义不同：根级 `test/` 只承载跨包 integration 与 workspace smoke，各宿主 `test/` 仍承载宿主内原子测试。
...
- 这条通道用于单包缓存、`--filter`、受影响范围验证与执行预览，不替代需要全仓收口时的根级 `pnpm verify`
...
- `verify:affected` 默认先执行治理门槛与受影响范围 Spec，再执行受影响包的 Type / Unit / Demo / Integration，最后执行 workspace-level integration。它是日常本地交付与 PR 前更常用的受影响范围入口。
...
- 更深的 Turbo 主链收敛仍需继续清理剩余任务边界与 `dependsOn` 编排；下一阶段治理路线见 [Turbo 循环依赖治理六阶段方案](/docs/packages/evals/turbo-cycle-reduction-stage-six-plan.md)
...
- `verify:governance` 只覆盖治理门槛，不替代受影响范围验证或根级 `pnpm verify`
- 日常开发需要完成验证时，优先使用新增测试、受影响测试、对应类型检查与必要治理检查；全量 `pnpm verify` 只在本地提交链路、main CI，或开发者明确需要全仓证明时使用，不作为每次修改代码后的默认动作
- 后续若要继续把 `typecheck`、`test:unit`、`test:integration` 精准迁入 Turbo，必须先处理当前循环依赖并统一 `apps/*`、`agents/*`、`packages/*` 的任务边界；详见 [Turbo 循环依赖治理六阶段方案](/docs/packages/evals/turbo-cycle-reduction-stage-six-plan.md)
...
- 日常改动不一定每次都跑完整 `pnpm check` 或全量 `pnpm verify`，但交付前应至少覆盖受影响范围所需的验证组合；全量 `pnpm verify` 主要用于本地提交链路触发的聚合验证、main CI 全量验证，以及开发者明确需要全仓证明的场景
...
- PR 增量校验：`/.github/workflows/pr-check.yml`
  - 代码改动默认按 `pnpm verify:affected` 的层级执行：并行跑 `verify:governance + test:spec:affected + lint:prettier:affected + lint:eslint:affected + typecheck:affected + test:unit:affected + test:demo:affected + test:integration:affected + test:workspace:integration:affected + test:workspace:smoke`
...
8. 如走受影响范围验证，优先确认 `VERIFY_BASE_REF` 是否正确；不要把全量 `pnpm verify` 作为每次文件修改后的默认动作
9. 同步更新文档
