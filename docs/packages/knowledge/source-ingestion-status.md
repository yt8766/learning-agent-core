...
最后核对：2026-05-02（确认知识库暂不做真实网页抓取）
...
- backend runtime center 通过 `ingestLocalKnowledge()` / `readKnowledgeOverview()` 消费本地 snapshot；user upload 的 workspace 文件读取 adapter、catalog sync / web curated / connector sync 的上游 entries adapter 已有，未在本轮范围内接入真实 connector API 同步 job；web curated 不再把真实网页抓取作为待补项。
