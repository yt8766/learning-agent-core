...
最后核对：2026-05-08（确认 local knowledge snapshot 已支持 repository 注入，知识库暂不做真实网页抓取）
...
- `ingestLocalKnowledge()`、`readKnowledgeOverview()`、`listKnowledgeArtifacts()` 与 `buildKnowledgeDescriptor()` 已支持注入 `LocalKnowledgeStoreOptions.repository` / `sourceProvider` / `runtimePaths`。新 runtime host 应优先注入 snapshot repository 与展示根路径，避免 runtime search / local snapshot 继续读写仓库根 `data/knowledge` 或展示为 `data/runtime`。未注入 repository 时的 filesystem snapshot 仅是 root data deprecation Phase 5 过渡 fallback。
- `ingestLocalKnowledge()` 的本地 embedding 写入在没有 `embeddings.apiKey`、`mcp.bigmodelApiKey` 或 `zhipuApiKey` 时必须直接写入 failed embedding receipt，不加载 `@agent/adapters`。这是为了让无凭据的本地/CI ingestion 测试保持确定、快速，并避免把既有循环包依赖放大成超时。
...
| ------------------------- | ---------------------------------------------------------------------------- | -------------------- | ----------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| repo docs                 | `repo-docs`                                                                  | 已有                 | 已有                    | 已有，`docs/**/*.md`；snapshot repository 可由 host 注入                     | backend runtime center 会调用 `ingestLocalKnowledge()` 读取本地 snapshot                                                                                      | 当前最明确，仍需按具体部署确认 embedding 凭据、snapshot repository 与 searchable 状态 |
...
| web curated               | `web-curated`                                                                | 已有                 | 已有                    | 不主动枚举；由人工策展或外部系统提供已整理 entries                           | backend 已有上游 entries adapter：`POST /api/platform/knowledge/sources/web-curated/ingest` -> builder -> source/chunk/receipt/vector；当前不接真实网页抓取器 | 保留为外部/人工已整理内容入口；真实抓取器 / robots / 版权策略 / 调度暂不做            |
...
- user upload 已有 workspace 内文件读取 adapter，catalog sync、web curated 与 connector sync 已有上游 entries adapter；真实 connector API 同步、权限 metadata 与 vendor response 清洗仍待上游 job 继续补齐。web curated 当前只接收已整理内容，不规划真实网页抓取器。
...
- `apps/backend/agent-server/src/runtime/services/runtime-knowledge.service.ts`：`ingestWebCuratedSources()` 接收上游已整理 curated URL entries，调用 `buildWebCuratedKnowledgePayload()` 后进入同一 ingestion facade。
- `apps/backend/agent-server/src/runtime/domain/knowledge/runtime-web-curated-ingestion-job.ts`：已删除；当前产品决策是不接真实 HTTP/MCP 抓取器、robots / 版权策略或抓取调度，web curated 只通过已整理 entries adapter 进入 ingestion。
...
- `packages/knowledge/test/local-knowledge-store.test.ts`：覆盖本地 docs/manifest ingestion snapshot、缺凭据 embedding fallback，以及 memory snapshot repository 不读写 root `data/knowledge` / `data/runtime` 的新路径；没有覆盖 user upload、web curated、catalog sync。
...
- backend runtime center 通过 `ingestLocalKnowledge()` / `readKnowledgeOverview()` 消费本地 snapshot；新 host 接线应注入 snapshot repository，未注入时才使用 root `data/knowledge` legacy fallback。user upload 的 workspace 文件读取 adapter、catalog sync / web curated / connector sync 的上游 entries adapter 已有，未在本轮范围内接入真实 connector API 同步 job；web curated 不再把真实网页抓取作为待补项。
...
- web curated 暂不做真实网页抓取；保留 `web-curated` sourceType、payload builder、HTTP facade 和 adapter，用于人工策展或外部系统已经完成抓取/清洗/安全判定后的内容产物。
