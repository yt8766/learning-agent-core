...
- `packages/platform-runtime` 的部分 registry / helper
...
- `packages/runtime` 是 Runtime Kernel
  ...
  - 不认识任何具体官方 agent
- `packages/platform-runtime` 是官方组合根
  - 负责官方 agent registry
  - 负责官方 `RuntimeAgentDependencies`
...
- `apps/*` 是启动适配器
  - backend / worker 只选择组合根并消费 facade
  - app 层不得重新拼官方 registry、workflow、runtime bootstrap
...
- `PlatformRuntimeFacade` 显式暴露：
  ...
  - `agentDependencies`
  - `metadata`
- `createOfficialRuntimeAgentDependencies()` 默认优先按 capability contract 解析官方模块，再回退兼容 `official.*` id
...
- `platform-runtime` 的 facade contract 需要持续维护，不能再退回“只导出几个 helper”

## 后续执行

- 继续把 backend / worker 中残留的官方 helper 读取面迁回 facade
