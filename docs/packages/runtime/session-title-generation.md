...
- 新会话默认标题仍为 `新会话`；`codex-chat` 前端显示的 `新对话` 也属于占位标题。
- 新会话会记录 `titleSource: "placeholder"`；即使调用方显式传入 `title: "新对话"`，runtime 也不能把它当作 `manual`。自动摘要成功或本地兜底派生后记录 `titleSource: "generated"`。
...
- 只有 `titleSource` 缺省或为 `placeholder` 且标题仍为空 / `新会话` / `新对话` 时，runtime 才能自动生成标题。
