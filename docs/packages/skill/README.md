...
- 公开入口：
  - 根入口：`@agent/skill`
  - `src/contracts/skill-facade.ts` 当前作为包根稳定导出的 facade contract
  - 当前真实宿主：
  ...
  - `@agent/skill` 根入口当前先通过 `contracts/skill-facade.ts` 导出上述 canonical host；legacy 根文件 `src/skill-registry.ts` 与 `src/agent-skill-loader.ts` 已删除
...
- `packages/skill` 的专项文档统一放在 `docs/packages/skill/`
...
- [package-structure-guidelines.md](/docs/packages/skill/package-structure-guidelines.md)

## Skill Draft Repository

`packages/skill/src/drafts/repository.ts` 当前提供两个 draft repository：
