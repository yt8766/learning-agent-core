...
- 职责：
  - tool registry、executor、approval preflight、MCP transport、通用 scaffold 执行能力
...
- 禁止：
  ...
  - 长期混入垂直领域生成引擎
- 依赖方向：
  - 只依赖 `@agent/core`、模板资产与必要第三方库
  - 禁止依赖或导入 `@agent/runtime`；runtime 可以依赖 tools，tools 不得反向依赖 runtime
  - 不得依赖任意 `agents/*`
- 公开入口：
  - 根入口：`@agent/tools`
- 约定：
  ...
  - `runtime-governance/` 只保留 tools 侧 runtime-governance executor repository；tools center 纯投影与 connector governance snapshot mutation 的真实宿主是 `@agent/runtime`
  ...
  - `watchdog/index.ts` 已删除；`ExecutionWatchdog` 的真实宿主是 `@agent/runtime`，`@agent/tools` 根入口不再转发
  - sandbox executor/provider 的真实宿主是 `@agent/runtime`；`@agent/tools` 仅保留自身 filesystem helper 与 executor 能力
