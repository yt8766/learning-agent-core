...
- `archive_thread`、`schedule_cancel`、`recover_run` 与 `list_runtime_artifacts` 通过 `RuntimeGovernanceRepository` 读写 archives、recoveries、cancellations 与 schedules 汇总；浏览器 replay 仍只读 `data/browser-replays` 目录。
