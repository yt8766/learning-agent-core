...
状态：current
文档类型：reference
适用范围：`packages/core`、`packages/report-kit`、`agents/data-report`、`apps/backend/agent-server`、`/Users/dev/Desktop/gosh_admin_fe`
最后核对：2026-04-24
...
- Bundle schema：`packages/core/src/data-report/schemas/data-report-json-bundle-schema.ts`
- Bundle types：`packages/core/src/data-report/types/data-report-json-bundle-schema.ts`
...
`packages/report-kit` 可以依赖 `@agent/core` 读取稳定 contract；`agents/data-report` 负责 LLM graph 编排；`apps/backend/*` 仍只能通过 facade 调用，不得直接重建 JSON 子流程。
...
1. `packages/core` 的稳定 schema
2. `agents/data-report` 的 flow / prompt / schema
3. `packages/report-kit` 的 renderer / writer
