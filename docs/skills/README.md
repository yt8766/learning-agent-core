...
这里描述的是给 Codex / Claude Code 这类代码代理读取的仓库技能，不是 `packages/skill` 的运行时技能系统。

包边界：

- 职责：
  - 仓库级代理技能的目录规范、术语边界与使用约定
  - `.agents/skills/*` 与 `packages/skill` 的语义区分
- 允许：
  - 代理技能目录结构说明
...
- 依赖方向：
  ...
  - 运行时技能系统请改看 [docs/packages/skill/README.md](/docs/packages/skill/README.md)

约定：

- `.agents/skills/*` 的专项说明统一放在 `docs/skills/`
- `packages/skill` 的包级实现与结构规范统一放在 `docs/packages/skill/`
