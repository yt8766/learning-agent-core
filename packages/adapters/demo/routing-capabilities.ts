...
async function main() {
  ...
  for (const model of llm.supportedModels()) {
    console.log(`- ${model.providerId}/${model.id}: ${model.capabilities.join(', ')}`);
  }
  console.log('plain text route:', plainTextResult);
  console.log('tool-call constrained route:', toolCallResult);
}
