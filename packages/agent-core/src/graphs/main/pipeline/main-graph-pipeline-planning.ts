...
} from './main-graph-pipeline-planning-helpers';
import { buildContextFilterAudienceSlices, orderRuntimeDispatches } from './main-graph-pipeline-dispatch-helpers';
...
export async function runDispatchStage(
  ...
  const filteredContextSlice = {
    summary: [task.goal, task.context, task.planDraft?.summary, task.plan?.summary]
      .filter(Boolean)
      .join(' | ')
      .slice(0, 240),
