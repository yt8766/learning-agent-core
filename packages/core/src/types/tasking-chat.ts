...
import { PlanQuestionRecordSchema } from './tasking-planning';
...
const ApprovalScopeSchema = z.enum(['once', 'session', 'always']);
const CapabilityCatalogItemSchema = z.object({
  id: z.string(),
...
const CapabilityCatalogGroupSchema = z.object({
  key: z.string(),
  label: z.string(),
  kind: z.enum(['skill', 'connector', 'tool']),
  items: z.array(CapabilityCatalogItemSchema)
});
const SkillDraftContractSchema = z.object({
...
export const ChatMessageCardSchema = z.discriminatedUnion('type', [
  ...
  z.object({
    ...
    title: z.string(),
    summary: z.string().optional(),
    status: z.enum(['pending', 'answered', 'bypassed', 'aborted']).optional(),
  ...
  z.object({
    ...
    title: z.string(),
    summary: z.string().optional(),
    groups: z.array(CapabilityCatalogGroupSchema)
  }),
  z.object({
    ...
    status: z.string(),
    enabled: z.boolean(),
    contract: SkillDraftContractSchema.optional(),
