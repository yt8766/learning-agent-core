...
import { MemoryEventRecord, MemoryEvidenceLinkRecord, MemoryRecord, UserProfileRecord } from '@agent/core';
...
export async function listMemoryReflections(reflectionRepository: ReflectionRepository, memoryId?: string) {
  const reflections = await reflectionRepository.list();
  if (!memoryId) {
    return reflections;
  }
  return reflections.filter(reflection => reflection.memoryId === memoryId);
}
