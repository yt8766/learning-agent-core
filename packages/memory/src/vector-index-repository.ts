...
export class LocalVectorIndexRepository implements VectorIndexRepository {
  ...
  async upsertMemory(record: MemoryRecord): Promise<void> {
    ...
    const embedding = await this.embeddingProvider.embedQuery(memoryText(record));
    await this.upsertRecord({
      id: record.id,
      namespace: 'memory',
      embedding,
      updatedAt: new Date().toISOString(),
      sourceHash: hashText(memoryText(record)),
      metadata: {
  ...
  async upsertRule(record: RuleRecord): Promise<void> {
    ...
    const embedding = await this.embeddingProvider.embedQuery(ruleText(record));
    await this.upsertRecord({
      id: record.id,
      namespace: 'rule',
      embedding,
      updatedAt: new Date().toISOString(),
      sourceHash: hashText(ruleText(record)),
      metadata: {
        name: record.name,
        summary: record.summary,
  ...
  async rebuild(): Promise<void> {
    ...
    const texts = [
      ...memoryRecords.map(record => memoryText(record)),
      ...ruleRecords.map(record => ruleText(record)),
