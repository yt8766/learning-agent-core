...
interface CounselorExperimentSummary {
  selectedCounselorId: string;
  selectedVersion: string;
  taskCount: number;
  successCount: number;
...
interface CapabilityTrustProfileSummary {
  capabilityId: string;
  displayName: string;
  trustLevel: 'high' | 'medium' | 'low';
  trustTrend: 'up' | 'steady' | 'down';
...
export function summarizeCounselorExperiments(
  ...
  const grouped = new Map<string, CounselorExperimentSummary>();
  ...
  const fallbackProfiles = Array.from(
    tasks
      ...
      .reduce((acc, attachment) => {
        ...
        const next: CapabilityTrustProfileSummary = {
        ...
        return acc;
      }, new Map<string, CapabilityTrustProfileSummary>())
      .values()
  );
