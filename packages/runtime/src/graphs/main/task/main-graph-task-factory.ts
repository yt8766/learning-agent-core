...
} from '../../../capabilities/capability-pool';
  ...
  async createTaskRecord(
    ...
    const dataReportContract =
      workflowResolution.preset.id === 'data-report'
        ? buildDataReportContract(workflowResolution.normalizedGoal, dto.context)
        : undefined;
    const enrichedTaskContext = dataReportContract
      ? appendDataReportContext(dto.context, dataReportContract)
      : dto.context;
    ...
    const task: TaskRecord = {
      ...
      contextFilterState: {
        ...
        filteredContextSlice: {
          ...
          compressionApplied: Boolean(dto.conversationCompression?.summary),
          compressionSource: dto.conversationCompression?.source,
          compressedMessageCount: dto.conversationCompression?.condensedMessageCount
        },
