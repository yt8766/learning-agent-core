import { buildModelHeatmap } from './runtime-analytics';
import { toCritiqueStyleReviewOutcome } from './runtime-center-projection.helpers';
import type { BuildRuntimeCenterProjectionInput } from './runtime-center-projection.types';

export function buildRuntimeCenterProjection(input: BuildRuntimeCenterProjectionInput) {
  ...
  const waitingInterruptTasks = input.tasks.filter(
    task => task.activeInterrupt?.status === 'pending' && task.activeInterrupt?.createdAt
  );
  ...
  return {
    ...
    waitingInterruptAverageMinutes:
      waitingInterruptTasks.reduce(
        (sum, task) => sum + (Date.now() - new Date(task.activeInterrupt!.createdAt as string).getTime()) / 60_000,
        0
      ) / Math.max(1, waitingInterruptTasks.length),
    ...
    streamMonitor: input.filteredRecentRuns.slice(0, 8).map(task => {
      ...
      const streamStatus = checkpoint?.streamStatus as
        | { nodeLabel?: string; nodeId?: string; detail?: string; progressPercent?: number; updatedAt?: string }
        | undefined;
      return {
        taskId: task.id,
        goal: task.goal,
        currentNode: streamStatus?.nodeLabel ?? streamStatus?.nodeId ?? task.currentNode,
        detail: streamStatus?.detail ?? task.contextFilterState?.filteredContextSlice?.summary,
        progressPercent: streamStatus?.progressPercent,
        updatedAt: streamStatus?.updatedAt ?? task.updatedAt
      };
    }),
    recentAgentErrors: input.deriveRecentAgentErrors(input.tasks),
    ...
    executionSpans: input.filteredRecentRuns.slice(0, 20).map(task => ({
      ...
      ministries: (task.executionPlan?.executionMinistries ?? [])
        .map(ministry => input.getMinistryDisplayName(ministry) ?? ministry)
        .filter(Boolean),
