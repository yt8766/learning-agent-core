...
export class SessionCoordinatorStore {
  ...
  mergeAssistantCognitionSnapshot(
    sessionId: string,
    messageId: string,
    snapshot: ChatCognitionSnapshot
  ): ChatMessageRecord | undefined {
    const items = this.messages.get(sessionId) ?? [];
    const message = items.find(candidate => candidate.id === messageId);
    if (!message || message.role !== 'assistant') {
      return undefined;
    }
    message.cognitionSnapshot = snapshot;
    return message;
  }
