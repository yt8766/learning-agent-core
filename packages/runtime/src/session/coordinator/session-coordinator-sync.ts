import type { ChatCheckpointRecord, ChatCognitionSnapshot } from '@agent/core';
...
export function buildCheckpointCognitionSnapshot(checkpoint: ChatCheckpointRecord): ChatCognitionSnapshot | undefined {
  const chain = checkpoint.thoughtChain;
  const ts = checkpoint.thinkState;
  if ((!chain || chain.length === 0) && !ts) {
    return undefined;
  }
  return {
    thoughtChain: chain?.length ? structuredClone(chain) : [],
    thinkingDurationMs: ts?.thinkingDurationMs,
    capturedAt: new Date().toISOString(),
    thinkState: ts ? structuredClone({ ...ts, loading: false, blink: false }) : undefined
  };
}

function finalizeAssistantMessageCognitionSnapshot(
  ...
  store.mergeAssistantCognitionSnapshot(sessionId, assistantMessageId, snapshot);
}
...
export function syncCoordinatorTask(
  ...
  const shouldFinalizeCognitionSnapshot =
    task.status === TaskStatus.COMPLETED ||
    task.status === TaskStatus.FAILED ||
    task.status === TaskStatus.BLOCKED ||
    task.status === TaskStatus.CANCELLED;
  if (shouldFinalizeCognitionSnapshot) {
    finalizeAssistantMessageCognitionSnapshot(store, sessionId, checkpoint, boundAssistantMessageId);
  }
