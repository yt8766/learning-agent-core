...
import type { ScaffoldTemplateDefinition } from './types';
...
function resolveScaffoldTemplatesRootDir(): string {
  ...
  const resolved = candidates.find(candidate => existsSync(join(candidate, 'scaffold')));
...
function findScaffoldTemplatesRootFrom(startDir: string): string | undefined {
  ...
  while (true) {
    ...
    if (existsSync(join(candidate, 'scaffold'))) {
      return candidate;
    }
...
export function resolveScaffoldTemplateDir(templateId: string) {
  ...
  return join(resolveScaffoldTemplatesRootDir(), 'scaffold', template.directoryName);
}
